...
Cette séparation facilite les traitements ETL, les transformations avec dbt et la création des Data Marts utilisés pour l'analyse.
...
Elles enregistrent les événements et les mesures (production, consommation d'énergie, consommation d'eau, déchets et transports) qui servent au calcul des indicateurs de performance (KPI) et à l'alimentation des Data Marts.
...
Cette table constitue l'une des principales tables de faits de la plateforme.

Elle sert de base pour calculer les indicateurs de production ainsi que les indicateurs environnementaux rapportés à la quantité produite (énergie, eau, déchets et émissions de CO₂).
...
Les données sont utilisées pour calculer des indicateurs tels que :
...
Les données sont utilisées pour calculer :
...
Ce modèle constitue la base de données relationnelle utilisée par la plateforme.

Il est exploité par :

- le pipeline ETL développé en Python ;
- les transformations réalisées avec dbt ;
- l'orchestration du pipeline avec Apache Airflow ;
- les Data Marts analytiques ;
- les tableaux de bord Power BI.

Il constitue la référence technique pour la structure des données et garantit la cohérence entre les différentes étapes de la plateforme.